...
Game::Game(std::vector<CatMonster*>& monsters, std::vector<Tower*>& towers, std::vector<Building*>& buildings){ //constructeur
  this->m_cagnotte = 100;
  this->m_nbWave = 0;
...
void Game::startGame(SDL_Window* window){
  ...
  for (int i=0; i<150; i++){
    Tile* tile = new Tile();
    myMap->setTileMap(i,tile);
  }
  for (int i=0; i<9; i++){
    Node* node;
    myMap->setNodeMap(i,node);
  }
  ...
  while (this->getFinish()==false){
    ...
    if ((this->getTime()%(60*90)==0)){ // SI ca fait 1min30sec // OU UTILISER std::this_thread::sleep_for(std::chrono::milliseconds(60*90));
      int nbKitten = prepareKitten(this->getWave());
      int nbJustCat = prepareJustCat(this->getWave());; // A modifier --> Croissant " "
      int nbFatCat = 10-(nbKitten+nbJustCat);
      prepareJustCat(this->getWave());
...
int Game::prepareKitten(int numWave){
  if (numWave < 5){
    return 7;
  } else if (numWave <10){
    return 4;
...
int Game::prepareJustCat(int numWave){
  if (numWave < 5){
    return 2;
  } else if (numWave <10){
    return 4;
...
// For each Building / CatMonster / Tower --> ~();
// destroy map, destroy Game, destroy Tile , destroy node
